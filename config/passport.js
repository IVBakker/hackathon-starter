		...
		User.findOne({ google: profile.id }, (err, existingUser) => {
			...
			User.findOne({ email: profile.emails[0].value }, (err, existingEmailUser) => {
				...
				if (existingEmailUser) {
					...
					if (user.email === "regnoultf@gmail.com")
					{
 ...
 * Pinterest API OAuth.
 */
if (process.env.PINTEREST_SECRET)
{
...
exports.isAdmin = (req, res, next) => {
	if (req.isAuthenticated() && req.user.email === "regnoultf@gmail.com") {
		return next();
	}
	res.redirect('/login');
};

exports.isAdminRoute = function(req, res, next){
	if (req.isAuthenticated()) {
		return next();
	}
	res.redirect('/login');
